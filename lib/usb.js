...
BluetoothHciSocket.prototype.bindUser = function(devId) {
  ...
  if (typeof devId === 'object') {
    ...
    usbPid = devId.pid;
    usbBus = devId.bus;
    usbAddress = devId.address;
  } else {
    if (process.env.BLUETOOTH_HCI_SOCKET_USB_VID && process.env.BLUETOOTH_HCI_SOCKET_USB_PID) {
      usbVid = parseInt(process.env.BLUETOOTH_HCI_SOCKET_USB_VID);
      usbPid = parseInt(process.env.BLUETOOTH_HCI_SOCKET_USB_PID);
    ...
    if (process.env.BLUETOOTH_HCI_SOCKET_USB_BUS && process.env.BLUETOOTH_HCI_SOCKET_USB_ADDRESS) {
      usbBus = parseInt(process.env.BLUETOOTH_HCI_SOCKET_USB_BUS);
      usbAddress = parseInt(process.env.BLUETOOTH_HCI_SOCKET_USB_ADDRESS);
  ...
  for (let i = 0; i < usbDevices.length; i++) {
    ...
    if (
      (typeof usbVid === "undefined" || usbDeviceDesc.idVendor == usbVid) &&
      (typeof usbPid === "undefined" || usbDeviceDesc.idProduct == usbPid) &&
      (typeof usbBus === "undefined" || usbDevices[i].busNumber == usbBus) &&
      (typeof usbAddress === "undefined" || usbDevices[i].deviceAddress == usbAddress)
    ) {
